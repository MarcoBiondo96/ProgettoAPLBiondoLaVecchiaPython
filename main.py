...
api_riot=config.RIOT_API_KEY#Prelevazione dal file config delle api key relative alle 2 case video ludiche (quella della riot va aggionata ogni 24 ore all'interno del file config.py)
...
def richiesta_tft(nome_ev):
    return tft_player_data.process_player_data(api_riot, nome_ev)

@app.route('/lol/<nome_ev>')
def richiesta_lol(nome_ev):
    return lol_player_data.process_player_data(api_riot, nome_ev)
